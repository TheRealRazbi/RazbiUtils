...
The path to search for files is starting_path/templates_path/img"""
    ...
    def find_on_screen(self, image_path: str, screenshot=None, confidence=0.75, color=0):
        """
        The path to search for files is starting_path/templates_path/img
        ...
        :param color: int, 0 = grayscale, 1 or higher = RGB | grayscale is faster
        ...
        res = None
        try:
            res = list(pyautogui.locateAll(needle,
    ...
    def find_on_screen_bool(self, image_path: str, screenshot=None, confidence=0.75):
        """
        Identical to find_on_screen, but it returns a True if at least one element detected and False otherwise
        """
        if self.find_on_screen(image_path, screenshot, confidence=confidence):
            return True
        return False
